...
let ppm = PPM(ny, nx, Path.Combine(imagesDirectory, "chapter7-part2.ppm"))

let rec color (ray : Ray) (world : Hitable) =
    match world.Hit(ray, 0.001, Double.MaxValue) with
    | Some record ->
...
for j = ny-1 downto 0 do
    for i = 0 to nx-1 do
        ...
        col <- col ./ (float ns)
        col <- roundVector (255.0 .* mapVector sqrt col)
        ppm.WriteVector col
